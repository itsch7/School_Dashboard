...
export class AddEditTeachersScheduleComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddEditTeachersScheduleComponent>,
    public _commonService: CommonService
  ) {}
  ngOnInit(): void {
    this.type = this.dialogData.type;
    this.teacherData = this.teachersData.filter(
      (teacher) => teacher.id == this.dialogData.teacher_id
    );
  ...
  setupForm() {
    if (this.dialogData.type == 'create') {
      this.Schedule = this.fb.group({
        date: [
          this._commonService.formatDate(this.dialogData.eventSegment.date),
          Validators.required,
        ],
        class: [this.teacherData[0].classes[0], Validators.required],
        start_time: [
          moment(this.dialogData.start_time).format('HH:mm'),
          Validators.required,
        ],
        end_time: [
          moment(this.dialogData.end_time).format('HH:mm'),
          Validators.required,
        ],
  ...
  onSubmit() {
    ...
    if (this.Schedule.valid) {
      if (this.dialogData.type != 'create') {
        this.dialogRef.close({
          ...this.Schedule.value,
          id: this.dialogData.meta.id,
        });
      }
      {
        this.dialogRef.close({
          ...this.Schedule.value,
          id: this.dialogData.events.length + 1,
        });
      }
    }
  }
